#!/usr/bin/env python3
"""Release checks specific to Pantone Challenger V1.6.1."""
...
for path in required:
    require(path.exists(), f"Missing required V1.6.1 file: {path.relative_to(ROOT)}")

config_path = ROOT / "config" / "temporal_semantic_integrity.json"
if config_path.exists():
    config = json.loads(config_path.read_text(encoding="utf-8"))
    require(config.get("methodology_version") == "1.6.1", "Methodology version must be 1.6.1")
...
if ERRORS:
    print("V1.6.1 validation failed:")
    for error in ERRORS:
        print(f"- {error}")
    raise SystemExit(1)

print("V1.6.1 temporal and semantic integrity validation passed.")
